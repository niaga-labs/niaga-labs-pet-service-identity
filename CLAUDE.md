...
Needs the dev-infra stack: Postgres database `kilat_identity` -> `cd ~/Documents/dev-infra; ./dev.ps1 up kilat`.
...
`go test ./...` - 12 passing. Three of the six test files sit behind the `integration` tag.

## Where things are

- `cmd/server` - `cmd/migrate` - `internal/handler` HTTP - `internal/application` services - `internal/domain/{identity,referral}` - `internal/repository` GORM

## Worth knowing

- JWT_SECRET here must match every other service and the gateway, or tokens are rejected downstream.
